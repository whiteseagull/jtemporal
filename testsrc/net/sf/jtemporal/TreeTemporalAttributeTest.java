 ...
 * @version $Id: TreeTemporalAttributeTest.java,v 1.3 2004/12/12 20:23:26 tabeck Exp $
 */

public class TreeTemporalAttributeTest extends TestCase implements TestConstants
{

  OldTreeTemporalAttribute map;
  OldTreeTemporalAttribute map2;

  TreeMapCopy mapCopy;

  public TreeTemporalAttributeTest (String testName) {
    super(testName);
  }
...
//  map  = new TreeTemporalAttribute(String.class);
    map  = new OldTreeTemporalAttribute(String.class, mapCopy);

    map2 = new OldTreeTemporalAttribute(Comparable.class);
    assertNotNull (map);
    assertNotNull (map2);
    assertTrue (map != map2);
  }
  ...
  public void tearDown() {
    ...
    assertEquals(mapCopy.getTreeMap().size(), map.getInternalMap().size());
    assertEquals(mapCopy.getTreeMap(), map.getInternalMap());
   ...
   */
  public static void checkConsistency(TemporalAttribute tum) {
    int size = tum.size();
    Iterator it = tum.periodSet().iterator();
    Period p, p2 = null;
    for (int i=1; i<=size; i++) {
       if (!it.hasNext()) throw new IllegalStateException("Outer iterator has no next");
       p = (Period) it.next();
       Iterator it2 = tum.periodSet().iterator();
       while (it2.hasNext()) {
	  p2 = (Period) it2.next();
	  if (p.equals(p2)) continue;
	  if (p.overlaps(p2))
	    throw new IllegalStateException("Found overlapping periods. P:"+p+" p2:"+p2);
       }

       if (i==1) { //check first
	  if (!tum.firstPeriod().equals(p))
	    throw new IllegalStateException("First period does not match");
	  if (!tum.lastPeriod().equals(p2))
	    throw new IllegalStateException("Last period does not match");
	  if (!tum.firstInstant().equals(p.getStart()))
	    throw new IllegalStateException("First instant does not match");
	  if (!tum.lastInstant().equals(p2.getEnd()))
	    throw new IllegalStateException("Last instant does not match");
       }
    } //for
    if (it.hasNext()) throw new IllegalStateException("size does not match outer iterator");
  }
